...
export default function AccountPage() {
  ...
  const save = async (e: FormEvent) => { e.preventDefault(); if (!user) return; setSaving(true); setError(null); setMessage(null);
    ...
    const { data, error: saveError } = await supabase.from("profiles").update({ ...form, username, display_name: form.display_name.trim() || null, bio: form.bio.trim() || null }).eq("id", user.id).select("id, username, display_name, bio, avatar_url, banner_url, website_url, twitter_url, instagram_url, discord_url, rating, reviews, created_at").single();
    if (saveError) setError(saveError.code === "23505" ? "That username is already taken." : "Couldn’t save your profile."); else { setProfile(data); setForm((f) => ({ ...f, username: data.username })); setMessage("Profile saved successfully."); } setSaving(false);
  };
  ...
  return <main className="mx-auto max-w-6xl px-4 pb-32 pt-8"><header className="mb-8"><span className="mono-label text-[#93939f]">Your account</span><h1 className="mt-3 text-3xl font-medium tracking-tight md:text-4xl">Profile & account</h1><p className="mt-2 max-w-xl text-sm text-[#93939f]">Customize how buyers see you on larpings.com.</p></header>
    <div className="grid gap-6 lg:grid-cols-12"><section id="customize" className="lg:col-span-8 space-y-6"><form onSubmit={save} className="rounded-[14px] border border-[#222226] bg-[#111113] p-5 md:p-6"><div className="flex flex-wrap items-center gap-4 border-b border-[#222226] pb-6"><div className="flex h-16 w-16 overflow-hidden items-center justify-center rounded-full bg-[#ff0000]/10 text-xl font-semibold text-[#ff0000]">{form.avatar_url ? <img src={form.avatar_url} alt="" className="h-full w-full object-cover" /> : username.slice(0, 2).toUpperCase()}</div><div><p className="text-xl font-medium">@{username}</p><p className="mt-1 text-sm text-[#93939f]">{user?.email}</p></div></div>
      {error && <p className="mt-5 rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-300">{error}</p>}{message && <p className="mt-5 rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm text-emerald-300">{message}</p>}
      <div className="grid gap-4 pt-6 sm:grid-cols-2"><label className="text-sm">Username<input value={form.username} onChange={update("username")} className="mt-2 w-full rounded-lg border border-[#222226] bg-[#09090b] px-3 py-3 text-sm outline-none focus:border-[#ff0000]" /></label><label className="text-sm">Display name<input value={form.display_name} onChange={update("display_name")} placeholder="Your public name" className="mt-2 w-full rounded-lg border border-[#222226] bg-[#09090b] px-3 py-3 text-sm outline-none focus:border-[#ff0000]" /></label></div>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm">Avatar<input type="file" accept="image/*" onChange={(e) => void upload("avatar", e)} className="mt-2 block w-full text-xs text-[#93939f]" /></label><label className="text-sm">Banner<input type="file" accept="image/*" onChange={(e) => void upload("banner", e)} className="mt-2 block w-full text-xs text-[#93939f]" /></label></div>
