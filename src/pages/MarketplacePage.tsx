...
const PLATFORMS = [
  ...
  { id: "discord", label: "Discord", dot: "#5865F2", Icon: DiscordIcon },
];
      ...
      <Link to={`/listing/${item.id}`} className="bg-[#111113] border border-[#222226] rounded-[14px] p-5 flex flex-col gap-3 hover:border-[#333338] hover:-translate-y-0.5 transition-all group">
        ...
        <div>
          <div className="flex items-center gap-1.5">
            <span className="text-xl font-semibold tracking-tight">{['username', 'account'].includes(item.category) ? '@' : ''}{displayOffer(item)}</span>
        ...
        <div className="flex items-center gap-3 text-[#93939f] text-xs mt-auto">
          <span>{item.category === "fansign" ? item.details?.delivery_format ?? "Custom fansign" : item.category === "service" ? item.details?.service_group ?? "Service" : item.category}</span>
          ...
          >
            ...
            <div className="relative z-10 px-8 md:px-12 py-14">
              ...
              <span className="bg-[rgba(9,9,11,0.5)] text-[#93939f] font-mono font-medium text-[11px] tracking-[1.76px] uppercase inline-flex items-center gap-2 px-3 py-1.5 rounded-[8px] border border-[#222226]">
                ...
                Marketplace — live inventory
              </span>

              <h1 className="leading-none font-display font-medium text-[42px] md:text-[60px] tracking-[-1.8px] max-w-[672px] mt-5">
                Rare loot & sauce <span className="text-[#ff0000]">for sale.</span>
              </h1>
      ...
      <main className="max-w-screen-xl mx-auto pt-8 pb-24 px-4">
        <div className="flex gap-8">
          ...
          <div className="flex-1 min-w-0">
            {/* Category tabs */}
            <div className="flex gap-2 overflow-x-auto pb-4 mb-4 border-b border-[#222226] no-scrollbar">
              {[
                { id: "all", label: "All Drops" },
                { id: "username", label: "Usernames" },
                { id: "service", label: "Services" },
                { id: "fansign", label: "Fansigns" }
              ].map(({ id, label }) => (
                <button
                  key={id}
                  onClick={() => setCategoryFilter(id)}
                  className={`flex shrink-0 items-center gap-1.5 text-sm font-medium px-4 py-2 rounded-[10px] transition-colors ${categoryFilter === id ? "bg-zinc-800 text-white" : "text-[#93939f] hover:bg-[#111113]"}`}
                >
                  {label}
                </button>
              ))}
