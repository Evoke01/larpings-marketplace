...
export default function ReputationPanel({ profileId, profileHandle, compact = false }: { profileId: string; profileHandle?: string; compact?: boolean }) {
  ...
  const submitRep = async () => {
    ...
    if (note.trim().length < 20 || note.trim().length > 280) { setError("Rep note must be between 20 and 280 characters."); return; }
    setBusy(true);
  ...
  return <section aria-labelledby="reputation-title" className={`rounded-[14px] border border-[#222226] bg-[#111113] ${compact ? "mt-4 p-4" : "mt-12 p-6 md:p-8"}`}>
    ...
    {compact && <div className="mt-3 flex flex-wrap gap-2 text-[10px] text-[#93939f]"><span>{repCount} Rep</span><span>·</span><span>{vouchCount} Vouch</span>{profileHandle && <Link to={`/seller/${encodeURIComponent(profileHandle)}`} className="ml-auto text-[#b7b7c2] hover:text-white">View reputation →</Link>}</div>}
  </section>;
}
