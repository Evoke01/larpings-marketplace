...
const BadgeIcon = ({ type }: { type: string }) => {
  ...
  if (type === "sales_10") return <svg {...common}><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>;
...
export default function BadgePill({ badgeType, compact = false }: BadgePillProps) {
  ...
  const label = labels[type] ?? type.split("_").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
