...
const BadgeIcon = ({ type }: { type: string }) => {
  const tone = type === "top_seller" ? "text-amber-300" : type === "trusted_seller" ? "text-[#b7b7c2]" : type === "og" ? "text-[#93939f]" : /sales/.test(type) ? "text-yellow-300" : "text-[#93939f]";
  ...
  return (
    <span
      ...
      className={`inline-flex items-center gap-1.5 rounded-full border font-[Poppins,ui-sans-serif,system-ui,sans-serif] font-medium leading-none transition-colors ${
        ...
        isDexter
          ? "border-[#ff0000] bg-[#ff0000]/10 text-[#ff5555] shadow-[0_0_12px_rgba(255,0,0,0.14)]"
          : isVerified
